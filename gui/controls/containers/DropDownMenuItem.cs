...
namespace Yuusha.gui
{
    public class DropDownMenuItem : Control
    {
        ...
        public DropDownMenuItem(string name, string text, DropDownMenu menu, VisualKey visualKey, string onMouseDown)
        {
            m_name = name;
            m_text = text;
            m_dropDownMenu = menu;
        ...
        protected override void OnMouseDown(MouseState ms)
        {
            ...
            if (!m_onMouseDownSent && ms.LeftButton == ButtonState.Pressed)
            {
                ...
                catch (Exception e)
                {
                    ...
                    Utils.LogOnce("Failed OnMouseDown event for DropDownMenuItem [ " + m_name + " ]");
                }
            }
        }

    ...
    static public class GameMode
    {
        ...
        public static void DisplayGameText(string text, Enums.ETextType textType)
        {
            try
            {
                ...
                (gui.GuiManager.Sheets["IOKGame"]["GameTextScrollableTextBox"] as gui.ScrollableTextBox).AddLine(text, textType);
            }
            catch (Exception e)
            {
            ...
            try
            {
                if (m_cells.Count > 0)
                {
                    ...
                    for (int count = 0; count < m_cells.Count; count++) // move through each cell and update the map and mobs list
                    {
                        ...
                        spLabel = GuiManager.GetControl(Enums.EGameState.Game.ToString(), "Tile" + count.ToString()) as SpinelTileLabel;

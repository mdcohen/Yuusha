...
namespace Yuusha
{
	...
	public class BitmapFont : IDisposable
	{
        ...
        public static Dictionary<string, BitmapFont> ActiveFonts
        {
            get { return m_dictBitmapFonts; }
        }
		...
		public void Reset(GraphicsDevice device)
		{
            ...
            foreach (KeyValuePair<int, BitmapInfo> kv in m_dictBitmapID2BitmapInfo)
            {
                ...
                if (m_fLoadFromResource)
                {
                    ...
                    tex = Texture2D.FromStream(device, s);//, tcp) as Texture2D;
                }
                else
                {
                    tex = Texture2D.FromStream(device, new System.IO.FileStream(m_strPath + kv.Value.strFilename, System.IO.FileMode.Open));//, tcp);
                }
